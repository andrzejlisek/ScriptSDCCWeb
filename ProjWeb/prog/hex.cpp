...
std::string Hex::StringEncode(std::string Txt)
{
    for (int I = 0; I < Txt.size(); I++)
    {
        bool NonStdChar = true;
        ...
        if (NonStdChar)
        {
            ...
            if (ChrX < 256)
            {
                Txt.insert(I + 1, IntToHex8(ChrX));
                Txt[I] = '%';
                I += 2;
            }
            else
            {
                Txt.insert(I + 1, IntToHex16(ChrX));
                Txt[I] = '_';
                I += 4;
            }
        }
...
std::string Hex::StringDecode(std::string Txt)
{
    for (int I = 0; I < Txt.size(); I++)
    {
        if (Txt[I] == '%')
        {
            Txt[I] = HexToInt(Txt.substr(I + 1, 2));
            Txt.erase(I + 1, 2);
        }
        else
        {
            if (Txt[I] == '_')
            {
                Txt[I] = HexToInt(Txt.substr(I + 1, 4));
                Txt.erase(I + 1, 4);
            }
        }
    }
